...
export const slides = [
  ...
  // Why NNoM — bridge from float training to int8-on-chip. ──────────────────
  {
    ...
    notes: [
      ...
      '',
      'NNoM stands for Neural Network on Microcontroller. It is a high-level, lightweight inference library designed specifically for deploying neural networks on MCUs. You hand it a trained Keras model and one Python call (nnom_generate_model) emits a single weights.h, which we link straight into kws_nnom_main.c. Bit-identical inference on the host harness and on the SoC.',
      '',
      'Card 1 — layer-wise INT8 with power-of-2 scales. Hazard3 has no FPU. Every floating-point op would be a soft-float helper call into libgcc — large and slow. NNoM does everything in int8 weights / int8 activations / int32 accumulators, and it restricts the requantization scale per layer to a power of two. So requantize collapses to a single arithmetic shift instead of an idiv. No FPU needed, no soft-float, no surprise division latency. (NNoM also accelerates on top of ARM CMSIS-NN for Cortex-M cores; we are RISC-V so we do not use that path, but it is part of the runtime\'s reputation.)',
      '',
      'Card 2 — ping-pong static activation buffer. This is the property that lets NNoM live in 64 KB. NNoM analyses the network at codegen time, finds the largest activation pair, and allocates exactly TWO static buffers sized for that worst case. Every layer reads from one, writes the other, then swaps. No malloc, no C runtime, no tensor-arena to grow at runtime. What the linker reports is exactly what runs — no surprise OOM mid-inference, no fragmentation. Weights live in XIP flash and are streamed by the cache prefetcher (slide 22).',
      '',
      'Card 3 — onboard pre-compiled, not interpreted. NNoM does not parse a model file at boot; the model topology is baked into C at build time. Zero interpreter overhead at runtime, deterministic timing. NNoM also natively supports complex topologies — Inception, ResNet, DenseNet, RNN/GRU/LSTM — which is not relevant for our plain Conv1D Mel Compact, but means the runtime is not the constraint if the model grows.',
      '',
